...
/**
 * N-Body simulation Brute force parallel solution.
 * It uses a barrier to synchronize between calculating forces and moving the bodies.
 * Barrier also used as a synchronization between GUI and simulation.
 * 
 * Main class. Acts like a GUI. It will initialize bodies, calculate forces, move bodies and repaint the GUI.
 * It will also take time for one simulation.
 * 
...
public class GUI extends JFrame {
	...
	private class PaintFrame extends JPanel {
		...
		public void paint(Graphics g) {
			...
			for(Body b : sim.getBodies()) {
				...
				int x = (int) Math.round(b.getPositionX()*scale+universeRadius);
				int y = (int) Math.round(b.getPositionY()*scale+universeRadius);
				g.drawOval(x-1, y-1, 2, 2);
				
				// If you want to see velocity vectors
				...
				// If you want to see the distance
	...
	public static void main(String[] args) {
		...
		int numSteps = Integer.parseInt(JOptionPane.showInputDialog(null,"Enter number of calculations(Number of calculate forces and moving)"));
		int amountWorkers = Integer.parseInt(JOptionPane.showInputDialog(null,"Enter number of worker threads"));
		Simulation sim = new Simulation(amountWorkers, gnumBodies, numSteps);
		new GUI(sim);
	}
